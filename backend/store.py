...
class Store:
    ...
    def invalidate_catalog(self):
        self.set("catalog_version", int(self.get("catalog_version")) + 1)

    def prune(self):
        cutoff = (datetime.now(timezone.utc) - timedelta(days=self.config.face_retention_days)).isoformat()
        with self.db:
            deleted = self.db.execute("DELETE FROM faces WHERE last_seen < ?", (cutoff,)).rowcount
    ...
    def observations(self, batch):
        ...
        groups = [{**dict(row), "vector": json.loads(row["embedding"])} for row in
                  self.db.execute("SELECT id, embedding, last_seen, quality FROM faces")]
        used = set()
        accepted = 0
        with self.db:
            for face, vector, jpeg in sorted(normalized, key=lambda item: item[0].confidence, reverse=True):
                ...
                if group is None or score < self.config.match_threshold:
                    if len(groups) >= self.config.max_faces:
                        continue
                    cursor = self.db.execute("""INSERT INTO faces
                        (embedding, thumbnail, quality, first_seen, last_seen, sightings, detection_confidence)
                        VALUES (?, ?, ?, ?, ?, 1, ?)""",
                        (json.dumps(vector), jpeg, face.quality, timestamp, timestamp, face.confidence))
    ...
    def faces(self, session_id, limit, offset):
        if session_id:
            source = "faces f JOIN face_sessions s ON f.id=s.face_id WHERE s.session_id=?"
            params = [session_id]
            fields = "f.id, s.first_seen, s.last_seen, s.sightings, f.detection_confidence, f.match_similarity"
            order = "s.last_seen"
        else:
            source, params = "faces f", []
        ...
        rows = self.db.execute(f"SELECT {fields} FROM {source} ORDER BY {order} DESC, f.id DESC LIMIT ? OFFSET ?",
                               [*params, limit, offset])
        return {"items": [{**dict(row), "label": f"Face {row['id']:03d}",
                           "thumbnail_url": f"/api/faces/{row['id']}/thumbnail"} for row in rows], "total": total}
