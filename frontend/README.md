...
- Signal Directory is an inline tab between Faces and Recordings in each stream's tool panel. It shares one directory poll across all views, while each panel keeps its own management target. Selecting an active feed switches Single view or selects a management target without changing the Multi-view grid; selecting archived history opens Single view. Add/Rename/Archive use scoped management forms with focus restoration. The former sidebar/drawer is removed. Changing tabs keeps players mounted and does not change recording or analysis.
