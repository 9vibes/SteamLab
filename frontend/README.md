...
- Signal Directory is closed by default behind the left sidebar button. It opens an accessible drawer containing stream selection, Add/Rename/Archive, and archived history. Selecting a stream closes the drawer; management actions replace it with their own form. Escape or Close restores focus to the sidebar. Mobile uses a compact horizontal entry and viewport-sized drawer. Opening/closing it leaves players mounted, keeps directory polling active, and does not change recording or analysis.
